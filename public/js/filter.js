document.addEventListener('DOMContentLoaded', function() {
  ...
  function getItemsPerPage() {
      ...
      // 전체게시판은 10개 유지 (기존 설정)
      if (currentPath.includes('/categories/') && !currentPath.includes('/youtube/')) {
          return 10;
      }
  ...
  const isYouTubeBoard = window.location.pathname.includes('/youtube/');
  ...
  function getOtherBoardFilteredItems() {
      ...
      if (isMainBoard) {
          ...
          // 로딩/에러 메시지 제외하고 실제 유튜브 아이템만 선택
          selector = '.youtube-item:not(.loading-message):not(.error-message):not(#youtube-loading):not(#youtube-error)';
      } else {
      ...
      // 빈 selector 체크
      if (!selector) {
          console.warn('Empty selector detected');
          return [];
      }
      ...
      return allItems.filter(item => {
          // 로딩/에러 메시지 요소 제외
          if (item.id === 'youtube-loading' || item.id === 'youtube-error' || 
              item.classList.contains('loading-message') || 
              item.classList.contains('error-message')) {
              return false;
          }
          ...
          if (currentSearchTerm) {
              ...
              if (isMainBoard || isblog) {
                  ...
                  titleSelector = '.youtube-title';
                  subtitleSelector = '.youtube-channel';
                  descSelector = '.youtube-date, .youtube-meta';
                  tagSelector = '';
                  categorySelector = '.youtube-type-badge, .category-overlay';
              } else {
              ...
              const categories = (dataCategory + ' ' + dataMain + ' ' + categoryText + ' ' + dataTitle).toLowerCase();
  ...
  function updateOtherBoardNoResultsMessage() {
      ...
      if (isMainBoard || isblog) {
          ...
          container = document.querySelector('.youtube-grid');
      } else {
